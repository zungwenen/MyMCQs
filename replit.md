...
- **Brand Logo**: Image logo (200x200px) displayed in header at appropriate sizes: h-10 md:h-12 (40px/48px) for user layout, h-9 (36px) with "Admin" badge for admin layout.
...
- **Scenario-Based Questions**: Full implementation with database schema, admin UI, and quiz interface. Scenarios contain passages with multiple related questions (MCQ, Boolean, Fill-in-gap). Admin UI features tabbed navigation with accordion view for scenario management. Quiz interface groups all scenario questions together under one accordion titled "Read this scenario and answer the questions that follow." Users must answer all questions in a scenario before proceeding. Questions are numbered within scenarios (1., 2., 3., etc.). Includes fallback handling for orphaned scenario references.
