...
- **Hero Section**: A compelling hero section is displayed for non-logged-in users, featuring law student imagery, key features, CTAs, and a subject preview. The "Get Started Free" CTA triggers the login dialog directly. Hero text emphasizes EasyRead IQ as the ultimate tool for Nigerian Law School modules and real-life legal issues, highlighting intelligent questions, model-based assessments, instant feedback, and one-time lifetime access.
...
- **Progressive Web App (PWA)**: Full PWA support with standalone display mode (no browser UI when installed), offline capability via service worker, platform-specific install prompts (native for Android/Chrome, Safari instructions for iOS/iPadOS including modern iPadOS with desktop UA detection), app icons in 8 sizes (72px-512px) using the brain logo with gradient design, and localStorage-based install state tracking.
